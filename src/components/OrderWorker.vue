...
import { apiLogin } from 'src/boot/axios' // apiLogin
...
const getWorker = async () => {
  const { data } = await apiLogin.post('/indeed/find/month', { year: date.value.split('-')[0], month: date.value.split('-')[1] })
...
const save = async () => {
  try {
    ...
    const result = await apiLogin.post('/indeed/create', form.value)
    console.log(result)
    $q.notify({
      message: '儲存成功',
      color: 'info',
...
const update = async () => {
  try {
    form.value.year = form.value.date.split('-')[0]
    form.value.month = form.value.date.split('-')[1]
    const result = await apiLogin.patch('/indeed/edit', form.value)
    console.log(result)
    $q.notify({
      message: '更新成功',
      color: 'info',
...
watchEffect(async () => {
  ...
  console.log(dateVal)
  const { data } = await apiLogin.post('/indeed/find/', { date: dateVal })
  await getWorker()
  console.log(data.data)
...
// watch three values to get needworker - available = cowork
watchEffect(() => {
  form.value.cowork = form.value.needworker - form.value.available
})
