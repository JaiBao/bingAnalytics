<template>
    <div  style="display: flex; gap: 20px; align-items: center;font-size: 25px;padding-bottom: 1em;padding-top: 1em">
        <q-btn-toggle
          v-model="browse"
    ...
    <div v-if="browse==='one'" style="font-size: 20px;">

      <div style="display: flex; gap: 20px; align-items: center">
        <div v-if="useType==='1'" style="padding-bottom: 3em">
          <q-select
              style="padding: 2px; width: 250px; font-size: 20px"
              filled
              v-model="selectBento"
      ...
      <div v-if="selectBento"  class="div-border">
          <q-select
              style="padding: 2px; width: 250px; font-size: 18px"
              filled
      ...
      <div v-if="selectProduct" >
        {{ "套餐總成本" }}
          <span class="primary" style="font-size: 20px;">  {{ totalAll }}</span>
          <br>
          <span style="font-size: 20px;"> {{"商品售價"  }} {{ totalPrice }}</span>
      </div>
    </div>
      <div style="display: flex; gap: 20px; align-items: center">
        <div v-if="selectBento"  class="div-border" >
    ...
    <div style="display: flex; gap: 20px; align-items: center">
      ...
      <!-- <q-btn @click="cleanBOM"  color="primary" style="font-size: 20px"  v-if="useType==='1'&&selectBento">{{ "清空" }}</q-btn> -->
       <q-btn @click="updatecombo" color="primary" style="font-size: 20px" v-if="useType==='1'&&selectBento">{{ "儲存" }}</q-btn>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    totalAll () {
      const num = (this.burrito6iTotalCost + this.drinkTotalCost + this.dish1TotalCost + this.dish2TotalCost + this.dish3TotalCost + this.dish4TotalCost + this.dish5TotalCost + this.dish6TotalCost + this.dish7TotalCost + this.dish8TotalCost + this.halfMainTotalCost)
    ...
    selectProduct (newVal, oldVal) {
      ...
      if (this.clean === false) {
        ...
        if (newVal.name === '素食便當' || newVal.name === '素排便當125元' || newVal.name === '素排便當150元') {
          console.log(this.bomData.filter(item => item.name.includes('素食')))
  ...
  methods: {
    ...
    updatecombo () {
      ...
      this.selectProduct.total = this.selectProduct.total.price
      const payload = {
        total: this.totalAll,
      ...
      const result = apiAuth.post('/analysis/update_combo', payload)
      console.log(result)
    },
    getProductCombo (id) {
      const payload = {
      ...
      apiAuth.post('/analysis/getBomProductItems', payload)
    },
    getCombo () {
      if (this.selectCombo) {
        ...
        this.selectProduct = getproduct[0].items.filter(item => item.name === '招牌便當')
        ...
        this.selectSide_dish1 = getproduct[0].items.filter(item => item.name === '廚娘油飯')
        this.selectSide_dish1 = this.selectSide_dish1[0]
        this.selectSide_dish2 = getproduct[0].items.filter(item => item.name === '鹽水煮蛋')
        this.selectSide_dish2 = this.selectSide_dish2[0]
        this.selectSide_dish3 = getproduct[0].items.filter(item => item.name === '香滷豆干')
        this.selectSide_dish3 = this.selectSide_dish3[0]
        this.selectSide_dish4 = getproduct[0].items.filter(item => item.name === '時蔬')
        this.selectSide_dish4 = this.selectSide_dish4[0]
        this.selectSide_dish5 = getproduct[0].items.filter(item => item.name === '梅汁蕃茄')
        this.selectSide_dish5 = this.selectSide_dish5[0]
        this.selectSide_dish6 = getproduct[0].items.filter(item => item.name === '酥炸菇菇')
        this.selectSide_dish6 = this.selectSide_dish6[0]
        this.selectSide_dish7 = getproduct[0].items.filter(item => item.name === '蛋塔')
        this.selectSide_dish7 = this.selectSide_dish7[0]
        this.dish1_quantity = 1
        this.dish2_quantity = 1
        this.dish3_quantity = 1
        this.dish4_quantity = 1
