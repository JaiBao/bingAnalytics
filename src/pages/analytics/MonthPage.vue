<template>
  ...
  <div v-if="canvas === '營收'" class="row w-100 q-mt-xl">
    ...
    <div class="row w-100">
      ...
      <q-select outlined v-model="district" :options="districtOption" class="col" />
    </div>
...
const dataset7 = ref([])
const dataset8 = ref([])
const district = ref({ label: '台北市', value: 2 })
const districtOption = ref([{ label: '台北市', value: 2 }, { label: '新北市', value: 3 }])

const selectDistrict = async (val, val2) => {
  const array = Object.entries(val)
  const find = val2.value
  console.log(find)
  await address.getcitysplice(find)
  if (find) {
    // 假設 address.city 是一個物件陣列，每個物件都有一個 label 屬性

    const result = array.filter((arrayItem) =>
      address.city.some((cityItem) => arrayItem[0] === cityItem.label)
    )

    const object = result.reduce((obj, [key, value]) => {
      obj[key] = value
      return obj
    }
    , {})
...
const get = async (val) => {
  try {
    ...
    const { data } = await apiShit.post('usermonth', {
    ...
    const result5 = await apiShit.post('state/month', {
      startDate: select.value.year + '-' + select.value.month,
      endDate
    })
    // const result5 = await axios.get(process.env.Fetch_api + 'price')
    const result6 = await apiShit.post('income', {
      startDate: select.value.year + '-' + select.value.month,
      endDate
    })
    ...
    dataset7.value = await selectDistrict(result5.data, district.value)
    dataset8.value = await selectDistrict(result6.data, district.value)
    // 篩選潤餅
